...
int battle_damage_area(struct block_list *bl, va_list ap) {
	...
	if (bl->type == BL_MOB && BL_UCAST(BL_MOB, bl)->class_ == MOBID_EMPERIUM)
		return 0;
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	switch( src->type ) { //Checks on actual src type
		...
		case BL_SKILL:
		{
			...
			break;
		case BL_MER:
			if (t_bl->type == BL_MOB && BL_UCAST(BL_MOB, t_bl)->class_ == MOBID_EMPERIUM && flag&BCT_ENEMY)
	...
	switch( s_bl->type ) { //Checks on source master
		case BL_PC:
		{
			...
			if (map_flag_gvg(m) && !sd->status.guild_id && t_bl->type == BL_MOB && BL_UCAST(BL_MOB, t_bl)->class_ == MOBID_EMPERIUM)
