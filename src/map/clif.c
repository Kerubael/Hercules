...
void clif_set_unit_idle2(struct block_list* bl, struct map_session_data *tsd, enum send_target target) {
	...
	p.effectState = sc != NULL ? sc->option : bl->type == BL_NPC ? BL_UCAST(BL_NPC, bl)->option : 0;
	p.job = vd->class_;
...
void clif_changeoption(struct block_list* bl)
{
	...
	WBUFL(buf,10) = sc != NULL ? sc->option : bl->type == BL_NPC ? BL_UCAST(BL_NPC, bl)->option : 0;
...
void clif_changeoption2(struct block_list* bl) {
	...
	WBUFW(buf,0) = 0x28a;
	WBUFL(buf,2) = bl->id;
	WBUFL(buf,6) = sc != NULL ? sc->option : bl->type == BL_NPC ? BL_UCAST(BL_NPC, bl)->option : 0;
...
void clif_joinchatok(struct map_session_data *sd,struct chat_data* cd)
{
	...
	if(cd->owner->type == BL_NPC) {
		struct npc_data *nd = BL_UCAST(BL_NPC, cd->owner);
		WFIFOL(fd, 30) = 1;
		WFIFOL(fd, 8) = 0;
...
void clif_charnameack (int fd, struct block_list *bl)
{
	...
	switch( bl->type ) {
		case BL_PC:
		{
			struct map_session_data *ssd = BL_UCAST(BL_PC, bl);
			struct party_data *p = NULL;
			struct guild *g = NULL;
			int ps = -1;
		...
		//[blackhole89]
		case BL_HOM:
			memcpy(WBUFP(buf,6), BL_UCAST(BL_HOM, bl)->homunculus.name, NAME_LENGTH);
			break;
		case BL_MER:
			memcpy(WBUFP(buf,6), BL_UCAST(BL_MER, bl)->db->name, NAME_LENGTH);
			break;
		case BL_PET:
			memcpy(WBUFP(buf,6), BL_UCAST(BL_PET, bl)->pet.name, NAME_LENGTH);
			break;
		case BL_NPC:
			memcpy(WBUFP(buf,6), BL_UCAST(BL_NPC, bl)->name, NAME_LENGTH);
			break;
		case BL_MOB:
		{
			struct mob_data *md = BL_UCAST(BL_MOB, bl);
		...
		case BL_CHAT:
			...
			memcpy(WBUFP(buf,6), BL_UCAST(BL_CHAT, bl)->title, NAME_LENGTH);
			break;
#endif
			return;
		case BL_ELEM:
			memcpy(WBUFP(buf,6), BL_UCAST(BL_ELEM, bl)->db->name, NAME_LENGTH);
			break;
		default:
...
void clif_changed_dir(struct block_list *bl, enum send_target target)
{
	...
	WBUFW(buf,0) = 0x9c;
	WBUFL(buf,2) = bl->id;
	WBUFW(buf,6) = bl->type == BL_PC ? BL_UCAST(BL_PC, bl)->head_dir : 0;
